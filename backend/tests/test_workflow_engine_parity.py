...
from app.graph import graph as langgraph_module
...
async def _collect_langgraph_steps(state):
    graph = langgraph_module.create_graph()
    steps = []
...
async def _collect_langgraph_steps_with_patch(monkeypatch, state, **patch_kwargs):
    _patch_both_engines(monkeypatch, **patch_kwargs)
    return await _collect_langgraph_steps(state)
...
def _patch_both_engines(monkeypatch, *, route="planner", review_sequence=None):
    ...
    for module in (python_engine_module, langgraph_module):
...
@pytest.mark.asyncio
async def test_python_engine_matches_langgraph_happy_path(monkeypatch):
    state = {"query": "hello", "search_mode": "hybrid", "revision_number": 0}

    assert await _collect_python_steps_with_patch(
        monkeypatch, state
    ) == await _collect_langgraph_steps_with_patch(monkeypatch, state)


@pytest.mark.asyncio
async def test_python_engine_matches_langgraph_rewrite_loop(monkeypatch):
    state = {"query": "hello", "search_mode": "hybrid", "revision_number": 0}
    patch_kwargs = {
        "review_sequence": [
    ...
    ) == await _collect_langgraph_steps_with_patch(monkeypatch, state, **patch_kwargs)


@pytest.mark.asyncio
async def test_python_engine_matches_langgraph_refine_path(monkeypatch):
    state = {"query": "改详细一点", "final_report": "old"}

    assert await _collect_python_steps_with_patch(
        monkeypatch, state, route="refiner"
    ) == await _collect_langgraph_steps_with_patch(monkeypatch, state, route="refiner")
